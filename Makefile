...
lint-py:  ## run python linter with ruff
	python -m ruff check fsspec_polars
	python -m ruff format --check fsspec_polars
...
fix-py:  ## fix python formatting with ruff
	python -m ruff check --fix fsspec_polars
	python -m ruff format fsspec_polars
...
test-py:  ## run python tests
	python -m pytest -v fsspec_polars/tests

# alias
tests-py: test-py

coverage-py:  ## run python tests and collect test coverage
	python -m pytest -v fsspec_polars/tests --cov=fsspec_polars --cov-report term-missing --cov-report xml
